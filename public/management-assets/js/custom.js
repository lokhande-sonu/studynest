    ...
    */
    (function() {
        ...
        $('.sa-datatables-init').each(function() {
            ...
            const tableId = $(this).attr('id') || $(this).closest('.card').find('table').index($(this)[0]);
            const stateKey = 'DataTables_' + window.location.pathname + '_' + tableId;

            // Check if we came from edit page (referrer contains /edit/)
            const cameFromEdit = document.referrer && document.referrer.includes('/edit/');

            // If not coming from edit page, clear the saved state
            if (!cameFromEdit) {
            ...
            const table = $(this).DataTable({
                ...
                stateLoadCallback: function(settings) {
                    if (!cameFromEdit) {
                        return null; // Don't restore state if not coming from edit
                    }
            ...
            if (tableSearchSelector) {
                ...
                // Clear search input if not coming from edit
                if (!cameFromEdit) {
                    $(tableSearchSelector).val('');
                }
            }
        });
    })();

...
@section('title', 'Order Transaction Report')
                    ...
                    <div class="container">
                        ...
                        <div class="py-5">
                            <div class="row g-4 align-items-center">
                                <div class="col">
                                    ...
                                    <h4 class="h4 m-0">Orders Payment Transaction Report</h4>
                                    <p class="mt-2">Filter and Export Orders Payment Transaction Data</p>
                                </div>
                        ...
                        <div class="card">
                            <div class="sa-chart-toolbar p-4">
                                    <div class="mb-4">
                                            <input type="text" placeholder="Search Orders by Order ID, Transaction ID, Customer Name, Customer Mobile Number"
                                            class="form-control form-control--search" id="table-search" />
                                        </div>
                                        
                                    <form method="GET" action="{{ route('management.transaction-report.index') }}" class="sa-chart-toolbar__body">

                                        <div class="sa-chart-toolbar__item me-auto">
                                            <label class="sa-chart-toolbar__item-label">Payment Date</label>
                                            <div class="d-flex">
                                        ...
                                        <div class="sa-chart-toolbar__item">
                                            <label class="sa-chart-toolbar__item-label">Payment Status</label>
                                            {{-- START: imohitmehto | 2026-08-25 | FIX: Payment filter labels now match actual DB values --}}
                                            {{-- 0=Failed, 1=Paid, 2=Pending --}}
                                            <select name="payment_status" class="form-select form-select-sm">
                                                <option value="">All</option>
                                                <option value="1" {{ request('payment_status')=='1'?'selected':'' }}>Paid</option>
                                                <option value="2" {{ request('payment_status')=='2'?'selected':'' }}>Pending</option>
                                                <option value="0" {{ request('payment_status')=='0'?'selected':'' }}>Failed</option>
                                            </select>
                                            {{-- END: imohitmehto | FIX: Payment filter label mapping --}}
                                        </div>
                                    
                                        <div class="sa-chart-toolbar__item">
                                            <button class="btn btn-primary btn-sm me-2">Filter</button>
                                    
                                            <a href="{{ route('management.transaction-report.export', request()->query()) }}"
                                               class="btn btn-success btn-sm me-2">
                                               Export Excel
                                            </a>
                                    
                                            <a href="{{ route('management.transaction-report.export-pdf', request()->query()) }}"
                            ...
                            <table class="sa-datatables-init" data-order="[[ 1, &quot;asc&quot; ]]"
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                       ...
                                       <td>{{ $order->order_payment_id }} via {{ \App\Enums\OrderPaymentMode::label($order->order_payment_mode) }}</td>
                                    </tr>
                                    @endforeach

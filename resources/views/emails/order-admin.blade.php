...
<p>A new order has been placed on StudyNest. Full order details are below.</p>
...
<p>
    <b>Name:</b> {{ $delivery['first_name'] ?? '' }} {{ $delivery['last_name'] ?? '' }}<br>
    <b>Email:</b> {{ $delivery['email'] ?? ($order->customer->cust_email ?? '') }}<br>
    <b>Phone:</b> {{ $delivery['phone'] ?? '' }}
    @if(!empty($order->order_gst_number))
        <br><b>GST Number:</b> {{ $order->order_gst_number }}
    @endif
</p>
...
<p>This order is awaiting processing in the admin dashboard.</p>

...
class OrderController extends Controller
{
    ...
    // 0=Failed, 1=Paid, 2=Pending (was inconsistent with 0=Pending before)
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        if ($request->has('status')) {
            ...
            // START: imohitmehto | 2026-08-25 | FIX: Stock only restored when cancelling a CONFIRMED order
            // Previously restored stock for any cancelled order, even PENDING/unpaid ones where stock was never decremented
            // Restore stock if order is being cancelled
            // Only restore if the order was CONFIRMED (stock was decremented)
            ...
            // END: imohitmehto | FIX: Stock restore only for CONFIRMED orders
    ...
    public function updatePaymentStatus(Request $request, $id)
    {
        ...
        if ($request->has('pay_status')) {
            ...
            // START: imohitmehto | 2026-08-25 | FIX: Payment status "Paid" now auto-sets order to CONFIRMED
            // Previously admin had to manually confirm after marking payment received
            if ($request->pay_status == 1) { // Paid
                $order->order_status = \App\Enums\OrderStatus::CONFIRMED;
            }
            // END: imohitmehto | FIX: Auto-confirm on payment received
    ...
    public function exportOrderReport()
    {
        ...
        foreach ($orders as $order) {
            ...
            $sheet->setCellValue('E' . $row, $this->paymentStatusLabel($order->order_payment_status));
            // START: imohitmehto | 2026-08-25 | FIX: Use OrderStatus::label() for human-readable status in exports
            // Previously used hardcoded numeric codes which showed raw integers in Excel/PDF
            $sheet->setCellValue('F' . $row, \App\Enums\OrderStatus::label($order->order_status));
            // END: imohitmehto | FIX: Human-readable status labels in exports
            $row++;
        }
